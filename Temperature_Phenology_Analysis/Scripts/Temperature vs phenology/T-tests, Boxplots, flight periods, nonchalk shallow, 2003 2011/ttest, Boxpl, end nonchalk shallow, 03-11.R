...
nrow(gis.data[gis.data$LC.SLOPE=="low lc.shallow slope",])
...
site.list<-unique(gis.data[gis.data$LC.SLOPE=="low lc.shallow slope",1])
site.list
length(site.list)
...
with(site.list.year, boxplot(daynum.nintieth~year,xlab="Year",ylab="Flight Period End Day"))
...
t.test(site.list.year$daynum.nintieth~site.list.year$year,paired=TRUE)
