...
start_date<-ggplot(phenology.gis.temperature, aes(x=JJA, y=daynum.tenth, colour = LC.SLOPE)) +
  geom_point(shape=16) +    
  geom_smooth(method=lm) + ylab("Day") + xlab("Temperature ?C") + ggtitle("Summer average temperature vs start date")+theme(legend.title=element_blank())
#start_date

end_date<-ggplot(phenology.gis.temperature, aes(x=JJA, y=daynum.nintieth, colour = LC.SLOPE)) +
  geom_point(shape=16) +    
  geom_smooth(method=lm)+ylab("Day")+xlab("Temperature ?C")+ ggtitle("Summer average temperature vs end date")+theme(legend.title=element_blank())
#end_date

mean_date<-ggplot(phenology.gis.temperature, aes(x=JJA, y=daynum.mean, colour = LC.SLOPE)) +
  geom_point(shape=16) +    
  geom_smooth(method=lm)+ylab("Day")+xlab("Temperature ?C")+ ggtitle("Summer average temperature vs mean date")+theme(legend.title=element_blank())
#mean_date

range_date <- ggplot(phenology.gis.temperature, aes(x = JJA, y = daynum.range, colour = LC.SLOPE)) +
  geom_point(shape=16) +    
  geom_smooth(method = lm) + ylab("Days") + xlab("Temperature ?C") + ggtitle("Summer average temperature vs flight period length") + theme(legend.title = element_blank())
#range_date
...
#start high steep
model.start.highsteep <- lm(daynum.tenth ~ JJA, data = phenology.high.steep)
#check assumptions
...
#start low shallow
model.start.lowshallow <- lm(daynum.tenth ~ JJA, data = phenology.low.shallow)
#check assumptions
...
# high steep
model.end.highsteep <- lm(daynum.nintieth ~ JJA, data = phenology.high.steep)
#check assumptions
#autoplot(model.end.highsteep)
#results on linear model
anova(model.end.highsteep)
summary(model.end.highsteep)

# low shallow
model.end.lowshallow <- lm(daynum.nintieth ~ JJA, data = phenology.low.shallow)
#check assumptions
#autoplot(model.end.lowshallow)
#results on linear model
anova(model.end.lowshallow)
summary(model.end.lowshallow)
...
# high steep
model.mean.highsteep <- lm(daynum.mean ~ JJA, data = phenology.high.steep)
#check assumptions
#autoplot(model.mean.highsteep)
#results on linear model
anova(model.mean.highsteep)
summary(model.mean.highsteep)

# low shallow
model.mean.lowshallow <- lm(daynum.mean ~ JJA, data = phenology.low.shallow)
#check assumptions
#autoplot(model.mean.lowshallow)
#results on linear model
anova(model.mean.lowshallow)
summary(model.mean.lowshallow)
...
# high steep
model.range.highsteep <- lm(daynum.range ~ JJA, data = phenology.high.steep)
#check assumptions
#autoplot(model.range.highsteep)
#results on linear model
anova(model.range.highsteep)
summary(model.range.highsteep)

# low shallow
model.range.lowshallow <- lm(daynum.range ~ JJA, data = phenology.low.shallow)
#check assumptions
#autoplot(model.range.lowshallow)
#results on linear model
anova(model.range.lowshallow)
summary(model.range.lowshallow)
