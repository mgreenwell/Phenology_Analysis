...
start_date<-ggplot(phenology.gis.temperature, aes(x=MAM, y=daynum.tenth, colour = LC.SLOPE)) +
  geom_point(shape=16) +    
  geom_smooth(method=lm)+ylab("Day")+xlab("Temperature ?C")+ ggtitle("Spring average temperature vs start date")+theme(legend.title=element_blank())
#start_date

end_date<-ggplot(phenology.gis.temperature, aes(x=MAM, y=daynum.nintieth, colour = LC.SLOPE)) +
  geom_point(shape=16) +    
  geom_smooth(method=lm)+ylab("Day")+xlab("Temperature ?C")+ ggtitle("Spring average temperature vs end date")+theme(legend.title=element_blank())
#end_date

mean_date<-ggplot(phenology.gis.temperature, aes(x=MAM, y=daynum.mean, colour = LC.SLOPE)) +
  geom_point(shape=16) +    
  geom_smooth(method=lm)+ylab("Day")+xlab("Temperature ?C")+ ggtitle("Spring average temperature vs mean date")+theme(legend.title=element_blank())
#mean_date

range_date<-ggplot(phenology.gis.temperature, aes(x=MAM, y=daynum.range, colour = LC.SLOPE)) +
  geom_point(shape=16) +    
  geom_smooth(method=lm)+ylab("Days")+xlab("Temperature ?C")+ ggtitle("Spring average temperature vs flight period length")+theme(legend.title=element_blank())
#range_date
...
#start high steep
model.start.highsteep <- lm(daynum.tenth ~ MAM, data = phenology.high.steep)
#check assumptions
...
#start low shallow
model.start.lowshallow <- lm(daynum.tenth ~ MAM, data = phenology.low.shallow)
#check assumptions
...
# high steep
model.end.highsteep <- lm(daynum.nintieth ~ MAM, data = phenology.high.steep)
#check assumptions
#autoplot(model.start.highsteep)
#results on linear model
...
# low shallow
model.end.lowshallow <- lm(daynum.nintieth ~ MAM, data = phenology.low.shallow)
#check assumptions
...
# high steep
model.mean.highsteep <- lm(daynum.mean ~ MAM, data = phenology.high.steep)
#check assumptions
...
# low shallow
model.mean.lowshallow <- lm(daynum.mean ~ MAM, data = phenology.low.shallow)
#check assumptions
##autoplot(model.start.lowshallow)
#results on linear model
...
# high steep
model.range.highsteep <- lm(daynum.range ~ MAM, data = phenology.high.steep)
#check assumptions
##autoplot(model.start.highsteep)
#results on linear model
...
# low shallow
model.range.lowshallow <- lm(daynum.range ~ MAM, data = phenology.low.shallow)
#check assumptions
#autoplot(model.start.lowshallow)
#results on linear model
