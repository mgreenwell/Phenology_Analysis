...
library(lme4)
model<-lmer(daynum.range~DEM_MEAN.SLOPE+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.range~DEM_MEAN.SLOPE))
...
#Testing whether DEM_MEAN.SLOPE has an effect on daynum.range by comparing with a model that just looks at daynum.range vs year with site as random effect
...
levels(phenology.gis$DEM_MEAN.SLOPE2)
model2<-lmer(daynum.range~DEM_MEAN.SLOPE2+year+(1|site),phenology.gis)
anova(model,model2)
...
levels(phenology.gis$DEM_MEAN.SLOPE3)<-c("A","B","A","C")     ## ("High Alt Shallow & Low Alt Shallow","High Alt Steep","High Alt Shallow & Low Alt Shallow","Low Alt Steep")
levels(phenology.gis$DEM_MEAN.SLOPE3)
model3<-lmer(daynum.range~DEM_MEAN.SLOPE3+year+(1|site),phenology.gis)
anova(model,model3)
...
levels(phenology.gis$DEM_MEAN.SLOPE4)
model4<-lmer(daynum.range~DEM_MEAN.SLOPE4+year+(1|site),phenology.gis)
anova(model3,model4)
...
levels(phenology.gis$DEM_MEAN.SLOPE5)
model5<-lmer(daynum.range~DEM_MEAN.SLOPE5+year+(1|site),phenology.gis)
anova(model3,model5)
...
levels(phenology.gis$DEM_MEAN.SLOPE6)
model6<-lmer(daynum.range~DEM_MEAN.SLOPE6+year+(1|site),phenology.gis)
anova(model3,model6)
...
levels(phenology.gis$DEM_MEAN.SLOPE7)
model7<-lmer(daynum.range~DEM_MEAN.SLOPE7+year+(1|site),phenology.gis)
anova(model3,model7)
...
levels(phenology.gis$DEM_MEAN.SLOPE8)
model8<-lmer(daynum.range~DEM_MEAN.SLOPE8+year+(1|site),phenology.gis)
anova(model3,model8)
...
levels(phenology.gis$DEM_MEAN.SLOPE9)<-c("A","B","A","B")
levels(phenology.gis$DEM_MEAN.SLOPE9)
model9<-lmer(daynum.range~DEM_MEAN.SLOPE9+year+(1|site),phenology.gis)
anova(model3,model9)
...
levels(phenology.gis$DEM_MEAN.SLOPE10)
model10<-lmer(daynum.range~DEM_MEAN.SLOPE10+year+(1|site),phenology.gis)
...
levels(phenology.gis$DEM_MEAN.SLOPE11)
model11<-lmer(daynum.range~DEM_MEAN.SLOPE11+year+(1|site),phenology.gis)
...
levels(phenology.gis$DEM_MEAN.SLOPE12)
model12<-lmer(daynum.range~DEM_MEAN.SLOPE12+year+(1|site),phenology.gis)
anova(model3,model12)
...
levels(phenology.gis$DEM_MEAN.SLOPE13)
model13<-lmer(daynum.range~DEM_MEAN.SLOPE13+year+(1|site),phenology.gis)
anova(model3,model13)
...
levels(phenology.gis$DEM_MEAN.SLOPE14)
model14<-lmer(daynum.range~DEM_MEAN.SLOPE14+year+(1|site),phenology.gis)
anova(model3,model14)


##########   have shown that simplest model is model 3 #########


############################### plotting model3 ###################################


with(phenology.gis, boxplot(daynum.range~DEM_MEAN.SLOPE3,xlab="Site type",ylab="daynum.range",ylim = c(0, 200)))
#plot boxplot of model3

library(plyr)
bargraph<-ddply(phenology.gis, c("DEM_MEAN.SLOPE3"),summarise,
                N=length(daynum.range),
                mean=mean(daynum.range),
                sd=sd(daynum.range),
                se=sd/ sqrt(N))
...
library(ggplot2)
??ggplot2
ggplot(bargraph, aes(x = DEM_MEAN.SLOPE3, y = mean)) +  
  geom_bar(position = position_dodge(), stat="identity",width=0.4,col="black",fill="grey") + 
...
p<-ggplot(phenology.gis,aes(x=DEM_MEAN.SLOPE3,y=daynum.range))+geom_violin(fill="grey",col="black")+theme_minimal()
...
test<-ddply(phenology.gis, c("DEM_MEAN.SLOPE3"),summarise,
            N=length(daynum.range),
            mean=mean(daynum.range),
            sd=sd(daynum.range),
            se=sd/ sqrt(N),
