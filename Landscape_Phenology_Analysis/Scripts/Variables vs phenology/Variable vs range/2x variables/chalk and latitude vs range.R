...
library(lme4)
model<-lmer(daynum.range~LC.LAT+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.range~LC.LAT))
...
ggplot(bargraph, aes(x = LC.LAT, y = mean)) +  
  ...
  expand_limits(y=c(0,60)) + scale_y_continuous(breaks=seq(0, 60, 10))
...
levels(phenology.gis$LC.LAT2)
model2<-lmer(daynum.range~LC.LAT2+year+(1|site),phenology.gis)
anova(model,model2)
...
levels(phenology.gis$LC.LAT3)
model3<-lmer(daynum.range~LC.LAT3+year+(1|site),phenology.gis)
anova(model,model3)
...
levels(phenology.gis$LC.LAT4)
model4<-lmer(daynum.range~LC.LAT4+year+(1|site),phenology.gis)
anova(model,model4)
...
levels(phenology.gis$LC.LAT5)
model5<-lmer(daynum.range~LC.LAT5+year+(1|site),phenology.gis)
anova(model,model5)
...
levels(phenology.gis$LC.LAT6)
model6<-lmer(daynum.range~LC.LAT6+year+(1|site),phenology.gis)
anova(model5,model6)
...
levels(phenology.gis$LC.LAT7)
model7<-lmer(daynum.range~LC.LAT7+year+(1|site),phenology.gis)
anova(model5,model7)
...
levels(phenology.gis$LC.LAT8)
model8<-lmer(daynum.range~LC.LAT8+year+(1|site),phenology.gis)
anova(model5,model8)
...
levels(phenology.gis$LC.LAT9)
model9<-lmer(daynum.range~LC.LAT9+year+(1|site),phenology.gis)
anova(model5,model9)
...
levels(phenology.gis$LC.LAT10)
model10<-lmer(daynum.range~LC.LAT10+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.LAT11)
model11<-lmer(daynum.range~LC.LAT11+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.LAT12)
model12<-lmer(daynum.range~LC.LAT12+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.LAT13)
model13<-lmer(daynum.range~LC.LAT13+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.LAT14)<-c("High chalk northern","Other","Other","Other")      #("B","A","A","A")     #("High chalk northern","Other","Other","Other")
levels(phenology.gis$LC.LAT14)
model14<-lmer(daynum.range~LC.LAT14+year+(1|site),phenology.gis)
anova(model5,model14)

##########   have shown that simplest model is model 14#########
with(phenology.gis, boxplot(daynum.range~LC.LAT14,xlab="Site type",ylab="Flight period length (days)"))

summary(LC.LAT14)
library(plyr)
bargraph<-ddply(phenology.gis, c("LC.LAT14"),summarise,
                N=length(daynum.range),
                mean=mean(daynum.range),
                sd=sd(daynum.range),
                se=sd/ sqrt(N))
...
library(ggplot2)
#??ggplot2
ggplot(bargraph, aes(x = LC.LAT14, y = mean)) +  
  geom_bar(position = position_dodge(), stat="identity",width=0.4,col="black",fill="grey") + 
...
p<-ggplot(phenology.gis,aes(x=LC.LAT14,y=daynum.range))+geom_violin(fill="grey",col="black")+theme_minimal()
...
test<-ddply(phenology.gis, c("LC.LAT14"),summarise,
            N=length(daynum.range),
            mean=mean(daynum.range),
            sd=sd(daynum.range),
            se=sd/ sqrt(N),
