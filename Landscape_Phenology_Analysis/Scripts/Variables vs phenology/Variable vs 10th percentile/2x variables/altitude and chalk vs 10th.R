...
library(lme4)
model<-lmer(daynum.tenth~LC.DEM+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.tenth~LC.DEM))
...
#Testing whether LC.DEM has an effect on daynum.tenth by comparing with a model that just looks at daynum.tenth vs year with site as random effect
...
levels(phenology.gis$LC.DEM3)<-c("High lc.high alt","high lc.low alt","low lc.high alt & low lc.low alt","low lc.high alt & low lc.low alt")
levels(phenology.gis$LC.DEM3)
model3<-lmer(daynum.tenth~LC.DEM3+year+(1|site),phenology.gis)
anova(model,model3)
...
levels(phenology.gis$LC.DEM2)
model2<-lmer(daynum.tenth~LC.DEM2+year+(1|site),phenology.gis)
anova(model,model2)
...
levels(phenology.gis$LC.DEM4)
model4<-lmer(daynum.tenth~LC.DEM4+year+(1|site),phenology.gis)
anova(model,model4)
...
levels(phenology.gis$LC.DEM5)<-c("B","C","A","A")          ##("High lc.high alt","high lc.low alt","low lc.high alt & low lc.low alt","low lc.high alt & low lc.low alt")
levels(phenology.gis$LC.DEM5)
model5<-lmer(daynum.tenth~LC.DEM5+year+(1|site),phenology.gis)
anova(model,model5)
...
levels(phenology.gis$LC.DEM6)
model6<-lmer(daynum.tenth~LC.DEM6+year+(1|site),phenology.gis)
anova(model5,model6)
...
levels(phenology.gis$LC.DEM7)
model7<-lmer(daynum.tenth~LC.DEM7+year+(1|site),phenology.gis)
anova(model5,model7)
...
levels(phenology.gis$LC.DEM8)
model8<-lmer(daynum.tenth~LC.DEM8+year+(1|site),phenology.gis)
anova(model5,model8)
...
levels(phenology.gis$LC.DEM9)
model9<-lmer(daynum.tenth~LC.DEM9+year+(1|site),phenology.gis)
anova(model5,model9)
...
levels(phenology.gis$LC.DEM10)
model10<-lmer(daynum.tenth~LC.DEM10+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM11)
model11<-lmer(daynum.tenth~LC.DEM11+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM12)
model12<-lmer(daynum.tenth~LC.DEM12+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM13)
model13<-lmer(daynum.tenth~LC.DEM13+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM14)<-c("B","A","A","A")
levels(phenology.gis$LC.DEM14)
model14<-lmer(daynum.tenth~LC.DEM14+year+(1|site),phenology.gis)
anova(model5,model14)

##########   have shown that simplest model is model 5 #########


############################### plotting model35 ###################################


with(phenology.gis, boxplot(daynum.tenth~LC.DEM3,xlab="Site type",ylab="daynum.tenth",ylim = c(0, 200)))
#plot boxplot of model2

library(plyr)
bargraph<-ddply(phenology.gis, c("LC.DEM5"),summarise,
                N=length(daynum.tenth),
                mean=mean(daynum.tenth),
                sd=sd(daynum.tenth),
                se=sd/ sqrt(N))
...
library(ggplot2)
#??ggplot2
ggplot(bargraph, aes(x = LC.DEM5, y = mean)) +  
  geom_bar(position = position_dodge(), stat="identity",width=0.4,col="black",fill="grey") + 
...
p<-ggplot(phenology.gis,aes(x=LC.DEM5,y=daynum.tenth))+geom_violin(fill="grey",col="black")+theme_minimal()
...
test<-ddply(phenology.gis, c("LC.DEM5"),summarise,
            N=length(daynum.tenth),
            mean=mean(daynum.tenth),
            sd=sd(daynum.tenth),
            se=sd/ sqrt(N),
