...
library(lme4)
model<-lmer(daynum.tenth~POINT_X.POINT_Y+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.tenth~POINT_X.POINT_Y))
...
#Testing whether POINT_X.POINT_Y has an effect on daynum.tenth by comparing with a model that just looks at daynum.tenth vs year with site as random effect
...
levels(phenology.gis$POINT_X.POINT_Y5)<-c("northern.eastern & southern.western","northern.western","southern.easton","northern.eastern & southern.western")
levels(phenology.gis$POINT_X.POINT_Y5)
model5<-lmer(daynum.tenth~POINT_X.POINT_Y5+year+(1|site),phenology.gis)
anova(model,model5)
...
levels(phenology.gis$POINT_X.POINT_Y2)<-c("A","B","C","A")     #("northern.eastern & southern.western","northern.western","southern.easton","northern.eastern & southern.western")
levels(phenology.gis$POINT_X.POINT_Y2)
model2<-lmer(daynum.tenth~POINT_X.POINT_Y2+year+(1|site),phenology.gis)
anova(model,model2)
...
levels(phenology.gis$POINT_X.POINT_Y3)
model3<-lmer(daynum.tenth~POINT_X.POINT_Y3+year+(1|site),phenology.gis)
anova(model2,model3)
...
levels(phenology.gis$POINT_X.POINT_Y4)
model4<-lmer(daynum.tenth~POINT_X.POINT_Y4+year+(1|site),phenology.gis)
anova(model2,model4)
...
levels(phenology.gis$POINT_X.POINT_Y6)
model6<-lmer(daynum.tenth~POINT_X.POINT_Y6+year+(1|site),phenology.gis)
anova(model2,model6)
...
levels(phenology.gis$POINT_X.POINT_Y7)
model7<-lmer(daynum.tenth~POINT_X.POINT_Y7+year+(1|site),phenology.gis)
anova(model2,model7)
...
levels(phenology.gis$POINT_X.POINT_Y8)
model8<-lmer(daynum.tenth~POINT_X.POINT_Y8+year+(1|site),phenology.gis)
anova(model2,model8)
...
levels(phenology.gis$POINT_X.POINT_Y9)
model9<-lmer(daynum.tenth~POINT_X.POINT_Y9+year+(1|site),phenology.gis)
anova(model2,model9)
...
levels(phenology.gis$POINT_X.POINT_Y10)
model10<-lmer(daynum.tenth~POINT_X.POINT_Y10+year+(1|site),phenology.gis)
...
levels(phenology.gis$POINT_X.POINT_Y11)
model11<-lmer(daynum.tenth~POINT_X.POINT_Y11+year+(1|site),phenology.gis)
...
levels(phenology.gis$POINT_X.POINT_Y12)
model12<-lmer(daynum.tenth~POINT_X.POINT_Y12+year+(1|site),phenology.gis)
...
levels(phenology.gis$POINT_X.POINT_Y13)
model13<-lmer(daynum.tenth~POINT_X.POINT_Y13+year+(1|site),phenology.gis)
...
levels(phenology.gis$POINT_X.POINT_Y14)
model14<-lmer(daynum.tenth~POINT_X.POINT_Y14+year+(1|site),phenology.gis)
anova(model2,model14)

##########   have shown that simplest model is model5 #########
...
with(phenology.gis, boxplot(daynum.tenth~POINT_X.POINT_Y5,xlab="Site type",ylab="daynum.tenth",ylim = c(0, 200)))
#plot boxplot of model2

library(plyr)
bargraph<-ddply(phenology.gis, c("POINT_X.POINT_Y5"),summarise,
                N=length(daynum.tenth),
                mean=mean(daynum.tenth),
                sd=sd(daynum.tenth),
                se=sd/ sqrt(N))
...
library(ggplot2)
#??ggplot2
ggplot(bargraph, aes(x = POINT_X.POINT_Y5, y = mean)) +  
  geom_bar(position = position_dodge(), stat="identity",width=0.4,col="black",fill="grey") + 
...
p<-ggplot(phenology.gis,aes(x=POINT_X.POINT_Y5,y=daynum.tenth))+geom_violin(fill="grey",col="black")+theme_minimal()
...
test<-ddply(phenology.gis, c("POINT_X.POINT_Y5"),summarise,
            N=length(daynum.tenth),
            mean=mean(daynum.tenth),
            sd=sd(daynum.tenth),
            se=sd/ sqrt(N),
