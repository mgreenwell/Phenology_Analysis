...
library(lme4)
model<-lmer(daynum.tenth~LC.LAT+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.tenth~LC.LAT))
...
levels(phenology.gis$LC.LAT2)
model2<-lmer(daynum.tenth~LC.LAT2+year+(1|site),phenology.gis)
anova(model,model2)
...
levels(phenology.gis$LC.LAT3)
model3<-lmer(daynum.tenth~LC.LAT3+year+(1|site),phenology.gis)
anova(model,model3)
...
levels(phenology.gis$LC.LAT4)
model4<-lmer(daynum.tenth~LC.LAT4+year+(1|site),phenology.gis)
anova(model,model4)
...
levels(phenology.gis$LC.LAT5)
model5<-lmer(daynum.tenth~LC.LAT5+year+(1|site),phenology.gis)
anova(model,model5)
...
levels(phenology.gis$LC.LAT6)
model6<-lmer(daynum.tenth~LC.LAT6+year+(1|site),phenology.gis)
anova(model,model6)
...
levels(phenology.gis$LC.LAT7)
model7<-lmer(daynum.tenth~LC.LAT7+year+(1|site),phenology.gis)
anova(model,model7)
...
levels(phenology.gis$LC.LAT8)
model8<-lmer(daynum.tenth~LC.LAT8+year+(1|site),phenology.gis)
anova(model,model8)
...
levels(phenology.gis$LC.LAT9)
model9<-lmer(daynum.tenth~LC.LAT9+year+(1|site),phenology.gis)
anova(model,model9)
...
levels(phenology.gis$LC.LAT10)
model10<-lmer(daynum.tenth~LC.LAT10+year+(1|site),phenology.gis)
anova(model,model10)
...
levels(phenology.gis$LC.LAT11)
model11<-lmer(daynum.tenth~LC.LAT11+year+(1|site),phenology.gis)
anova(model,model11)
...
levels(phenology.gis$LC.LAT12)
model12<-lmer(daynum.tenth~LC.LAT12+year+(1|site),phenology.gis)
anova(model,model12)
...
levels(phenology.gis$LC.LAT13)
model13<-lmer(daynum.tenth~LC.LAT13+year+(1|site),phenology.gis)
anova(model,model13)
...
levels(phenology.gis$LC.LAT14)
model14<-lmer(daynum.tenth~LC.LAT14+year+(1|site),phenology.gis)
anova(model,model14)

##########   have shown that simplest model is model #########
with(phenology.gis, boxplot(daynum.tenth~LC.LAT,xlab="Site type",ylab="Flight start day"))
...
p<-ggplot(phenology.gis,aes(x=LC.LAT,y=daynum.tenth))+geom_violin(fill="grey",col="black")+theme_minimal()
