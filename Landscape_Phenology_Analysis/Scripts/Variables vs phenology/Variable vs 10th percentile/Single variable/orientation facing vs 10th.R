...
library(lme4)
model<-lmer(daynum.tenth~phenology.gis$NORTHNESS_MEAN.label+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.tenth~phenology.gis$NORTHNESS_MEAN.label))
