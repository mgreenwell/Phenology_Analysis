...
gis.data$BgRoper<-gis.data$BgRo/(pi*500^2)*100            #Add row - convert LC to percentage of LC
#head(gis.data)
#summary(gis.data$LC)
...
summary(phenology.gis$BgRo)
nrow(phenology.gis)
#phenology.gis<-phenology.gis[(phenology.gis$BgRo >=0),]#remove values below 0
nrow(phenology.gis)
summary(phenology.gis$BgRoper)
phenology.gis$BgRoper.label[phenology.gis$BgRoper<(0.5033)]<-"low bare ground"  
phenology.gis$BgRoper.label[phenology.gis$BgRoper>(0.5033)]<-"high bareground"  
table(phenology.gis$BgRoper.label)


library(lme4)
model<-lmer(daynum.nintieth~phenology.gis$BgRoper.label+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.nintieth~phenology.gis$BgRoper.label))
...
library(plyr)
test<-ddply(phenology.gis, c("BgRoper.label"),summarise,
