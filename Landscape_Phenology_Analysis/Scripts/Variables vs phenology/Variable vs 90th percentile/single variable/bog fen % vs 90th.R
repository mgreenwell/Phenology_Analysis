...
gis.data$Fper<-gis.data$F/(pi*500^2)*100            #Add row - convert LC to percentage of LC
#head(gis.data)
#summary(gis.data$LC)
...
summary(phenology.gis$F)
nrow(phenology.gis)
#phenology.gis<-phenology.gis[(phenology.gis$F >=0),]#remove values below 0
nrow(phenology.gis)
summary(phenology.gis$Fper)
phenology.gis$Fper.label[phenology.gis$Fper<(0.6214)]<-"low bog"  
phenology.gis$Fper.label[phenology.gis$Fper>(0.6214)]<-"high bog"  
table(phenology.gis$Fper.label)


library(lme4)
model<-lmer(daynum.nintieth~phenology.gis$Fper.label+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.nintieth~phenology.gis$Fper.label))
...
library(plyr)
test<-ddply(phenology.gis, c("Fper.label"),summarise,
