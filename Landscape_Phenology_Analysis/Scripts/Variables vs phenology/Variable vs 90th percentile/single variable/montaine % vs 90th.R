...
gis.data$Mper<-gis.data$M/(pi*500^2)*100            #Add row - convert LC to percentage of LC
#head(gis.data)
#summary(gis.data$LC)
...
summary(phenology.gis$M)
nrow(phenology.gis)
#phenology.gis<-phenology.gis[(phenology.gis$M >=0),]#remove values below 0
nrow(phenology.gis)
summary(phenology.gis$Mper)
phenology.gis$Mper.label[phenology.gis$Mper<(0.209)]<-"low montaine"  
phenology.gis$Mper.label[phenology.gis$Mper>(0.209)]<-"high montaine"  
table(phenology.gis$Mper.label)


library(lme4)
model<-lmer(daynum.nintieth~phenology.gis$Mper.label+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.nintieth~phenology.gis$Mper.label))
...
library(plyr)
test<-ddply(phenology.gis, c("Mper.label"),summarise,
