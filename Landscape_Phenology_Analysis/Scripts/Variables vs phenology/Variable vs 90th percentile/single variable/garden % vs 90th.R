...
gis.data$UGper<-gis.data$UG/(pi*500^2)*100            #Add row - convert LC to percentage of LC
#head(gis.data)
#summary(gis.data$LC)
...
summary(phenology.gis$UG)
nrow(phenology.gis)
#phenology.gis<-phenology.gis[(phenology.gis$UG >=0),]#remove values below 0
nrow(phenology.gis)
summary(phenology.gis$UGper)
phenology.gis$UGper.label[phenology.gis$UGper<(8.213)]<-"low garden"  
phenology.gis$UGper.label[phenology.gis$UGper>(8.213)]<-"high garden"  
table(phenology.gis$UGper.label)


library(lme4)
model<-lmer(daynum.nintieth~phenology.gis$UGper.label+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.nintieth~phenology.gis$UGper.label))
...
library(plyr)
test<-ddply(phenology.gis, c("UGper.label"),summarise,
