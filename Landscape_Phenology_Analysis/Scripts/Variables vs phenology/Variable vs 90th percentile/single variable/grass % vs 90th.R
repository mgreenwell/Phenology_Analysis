...
gis.data$Gper<-gis.data$G/(pi*500^2)*100            #Add row - convert LC to percentage of LC
#head(gis.data)
#summary(gis.data$LC)
...
summary(phenology.gis$G)
nrow(phenology.gis)
#phenology.gis<-phenology.gis[(phenology.gis$G >=0),]#remove values below 0
nrow(phenology.gis)
#hist(phenology.gis$Gper,breaks=40,col="red",ylim =c(0,700),xlim =c(0,25), xlab = "Mean slope, degrees from horizontal",main="Histogram of mean slope angle")
summary(phenology.gis$Gper)
phenology.gis$Gper.label[phenology.gis$Gper<(32.94)]<-"low grass"  
phenology.gis$Gper.label[phenology.gis$Gper>(32.94)]<-"high grass"  
table(phenology.gis$Gper.label)


library(lme4)
model<-lmer(daynum.nintieth~phenology.gis$Gper.label+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.nintieth~phenology.gis$Gper.label))
...
library(plyr)
test<-ddply(phenology.gis, c("Gper.label"),summarise,
