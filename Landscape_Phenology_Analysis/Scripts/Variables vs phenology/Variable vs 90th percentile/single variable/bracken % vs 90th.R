...
gis.data$Brper<-gis.data$Br/(pi*500^2)*100            #Add row - convert LC to percentage of LC
#head(gis.data)
#summary(gis.data$LC)
...
summary(phenology.gis$Br)
nrow(phenology.gis)
#phenology.gis<-phenology.gis[(phenology.gis$Br >=0),]#remove values below 0
nrow(phenology.gis)
summary(phenology.gis$Brper)
phenology.gis$Brper.label[phenology.gis$Brper<(0.5245)]<-"low bracken"  
phenology.gis$Brper.label[phenology.gis$Brper>(0.5245)]<-"high bracken"  
table(phenology.gis$Brper.label)


liBrary(lme4)
model<-lmer(daynum.nintieth~phenology.gis$Brper.label+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.nintieth~phenology.gis$Brper.label))
...
library(plyr)
test<-ddply(phenology.gis, c("Brper.label"),summarise,
