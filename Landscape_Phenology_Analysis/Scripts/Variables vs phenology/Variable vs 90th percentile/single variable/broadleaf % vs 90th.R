...
gis.data$BWper<-gis.data$BW/(pi*500^2)*100            #Add row - convert LC to percentage of LC
#head(gis.data)
#summary(gis.data$LC)
...
summary(phenology.gis$BW)
nrow(phenology.gis)
#phenology.gis<-phenology.gis[(phenology.gis$BW >=0),]#remove values below 0
nrow(phenology.gis)
summary(phenology.gis$BWper)
phenology.gis$BWper.label[phenology.gis$BWper<(23.03)]<-"low broadleaf"  
phenology.gis$BWper.label[phenology.gis$BWper>(23.03)]<-"high broadleaf"  
table(phenology.gis$BWper.label)


library(lme4)
model<-lmer(daynum.nintieth~phenology.gis$BWper.label+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.nintieth~phenology.gis$BWper.label))
...
library(plyr)
test<-ddply(phenology.gis, c("BWper.label"),summarise,
