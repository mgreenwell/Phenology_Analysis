...
phenology.gis$POINT_Y.label[phenology.gis$POINT_Y<(252727)]<-"Southern"  #using mean
phenology.gis$POINT_Y.label[phenology.gis$POINT_Y>(252727)]<-"Northern"  
table(phenology.gis$POINT_Y.label)


library(lme4)
model<-lmer(daynum.nintieth~phenology.gis$POINT_Y.label+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.nintieth~phenology.gis$POINT_Y.label))
