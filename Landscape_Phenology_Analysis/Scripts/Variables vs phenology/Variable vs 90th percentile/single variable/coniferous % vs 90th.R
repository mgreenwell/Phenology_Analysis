...
gis.data$CWper<-gis.data$CW/(pi*500^2)*100            #Add row - convert LC to percentage of LC
#head(gis.data)
#summary(gis.data$LC)
...
summary(phenology.gis$CW)
nrow(phenology.gis)
#phenology.gis<-phenology.gis[(phenology.gis$CW >=0),]#remove values below 0
nrow(phenology.gis)
summary(phenology.gis$CWper)
phenology.gis$CWper.label[phenology.gis$CWper<(5.374)]<-"low coniferous wood"  
phenology.gis$CWper.label[phenology.gis$CWper>(5.374)]<-"high coniferous wood"  
table(phenology.gis$CWper.label)


library(lme4)
model<-lmer(daynum.nintieth~phenology.gis$CWper.label+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.nintieth~phenology.gis$CWper.label))
...
library(plyr)
test<-ddply(phenology.gis, c("CWper.label"),summarise,
