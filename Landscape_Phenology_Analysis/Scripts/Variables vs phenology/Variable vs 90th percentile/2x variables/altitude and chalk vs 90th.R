...
library(lme4)
model<-lmer(daynum.nintieth~LC.DEM+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.nintieth~LC.DEM))
...
#Testing whether LC.DEM has an effect on daynum.nintieth by comparing with a model that just looks at daynum.nintieth vs year with site as random effect
...
levels(phenology.gis$LC.DEM2)<-c("A","B","B","B")
levels(phenology.gis$LC.DEM2)
model2<-lmer(daynum.nintieth~LC.DEM2+year+(1|site),phenology.gis)
anova(model,model2)

#no sig dif, model2 simpler
...
levels(phenology.gis$LC.DEM3)
model3<-lmer(daynum.nintieth~LC.DEM3+year+(1|site),phenology.gis)
anova(model,model3)
...
levels(phenology.gis$LC.DEM4)
model4<-lmer(daynum.nintieth~LC.DEM4+year+(1|site),phenology.gis)
anova(model,model4)
...
levels(phenology.gis$LC.DEM5)<-c("B","C","A","A")           ###this one
levels(phenology.gis$LC.DEM5)
model5<-lmer(daynum.nintieth~LC.DEM5+year+(1|site),phenology.gis)
anova(model,model5)
...
levels(phenology.gis$LC.DEM6)
model6<-lmer(daynum.nintieth~LC.DEM6+year+(1|site),phenology.gis)
anova(model5,model6)
...
levels(phenology.gis$LC.DEM7)
model7<-lmer(daynum.nintieth~LC.DEM7+year+(1|site),phenology.gis)
anova(model5,model7)
...
levels(phenology.gis$LC.DEM8)
model8<-lmer(daynum.nintieth~LC.DEM8+year+(1|site),phenology.gis)
anova(model5,model8)
...
levels(phenology.gis$LC.DEM9)
model9<-lmer(daynum.nintieth~LC.DEM9+year+(1|site),phenology.gis)
anova(model5,model9)
...
levels(phenology.gis$LC.DEM10)
model10<-lmer(daynum.nintieth~LC.DEM10+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM11)
model11<-lmer(daynum.nintieth~LC.DEM11+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM12)
model12<-lmer(daynum.nintieth~LC.DEM12+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM13)
model13<-lmer(daynum.nintieth~LC.DEM13+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM14)
model14<-lmer(daynum.nintieth~LC.DEM14+year+(1|site),phenology.gis)
...
with(phenology.gis, boxplot(daynum.nintieth~LC.DEM5,xlab="Site type",ylab="daynum.nintieth",ylim = c(0, 200)))
...
p<-ggplot(phenology.gis,aes(x=LC.DEM5,y=daynum.nintieth))+geom_violin(fill="grey",col="black")+theme_minimal()
