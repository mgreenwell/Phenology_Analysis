...
## Remove means of less than 80. check this.
  phenology.gis <- phenology.gis %>% filter(daynum.mean >= 80)
summary(phenology.gis$daynum.mean)
  ############################ allocating values #################################

#hist(phenology.gis$LCper, breaks = 40, col = "red",  ylim = c(0, 6000),  
#xlim = c(0, 100),  xlab = "Percentage of calcarious grassland cover",
#main = "Histogram of LC%")


## Chalk

summary(phenology.gis$LCper)
## Above mean = high, below mean = low
...
# hist(
#   phenology.gis$SLOPE_MEAN,
#   breaks = 40,
#   col = "red",
#   ylim = c(0, 700),
#   xlim = c(0, 25),
#   xlab = "Mean slope, degrees from horizontal",
#   main = "Histogram of mean slope angle"
# )
...
summary(phenology.gis$SLOPE_MEAN)
phenology.gis$SLOPE_MEAN.label[phenology.gis$SLOPE_MEAN < (5.03)] <-
  "shallow slope"
phenology.gis$SLOPE_MEAN.label[phenology.gis$SLOPE_MEAN > (5.03)] <-
  "steep slope"
...
for (i in 1:nrow(phenology.gis)) {
  phenology.gis$LC.SLOPE[i] <-
    paste(phenology.gis[i, "LC.label"], 
          phenology.gis[i, "SLOPE_MEAN.label"], sep =
            ".")
}
table(phenology.gis$LC.SLOPE)

## Run mixed effects model of mean vs chalk and slope + year as fixed effects
# Site as a random effect
# (1|site) gives site as random effect- not a factor of interest
model <- lmer(daynum.mean ~ LC.SLOPE + year + (1 | site), phenology.gis)

summary(model)

#with(phenology.gis, boxplot(daynum.mean ~ LC.SLOPE))
...
#Testing whether LC.SLOPE has an effect on daynum.mean by comparing with a model that just looks at daynum.mean vs year with site as random effect
...
levels(phenology.gis$LC.SLOPE2) <-
  c("High chalk shallow slope & low chalk steep slope",
    "high chalk steep slope",
    "low chalk shallow slope",
    "High chalk shallow slope & low chalk steep slope")
#("High Lc shallow & low LC steep","high LC steep",low LC shallow","High Lc shallow & low LC steep")
levels(phenology.gis$LC.SLOPE2)
...
# levels(phenology.gis$LC.SLOPE3)
# model3 <- lmer(daynum.mean ~ LC.SLOPE3 + year + (1 |
#                                                    site), phenology.gis)
# anova(model2, model3)
# 
# #######
...
# levels(phenology.gis$LC.SLOPE4)
# model4 <- lmer(daynum.mean ~ LC.SLOPE4 + year + (1 |
#                                                    site), phenology.gis)
# anova(model2, model4)
# 
# #######
...
# levels(phenology.gis$LC.SLOPE5)
# model5 <- lmer(daynum.mean ~ LC.SLOPE5 + year + (1 |
#                                                    site), phenology.gis)
# anova(model2, model5)
# 
# #######
...
# levels(phenology.gis$LC.SLOPE6)
# model6 <- lmer(daynum.mean ~ LC.SLOPE6 + year + (1 |
#                                                    site), phenology.gis)
# anova(model2, model6)
# 
# #######
...
# levels(phenology.gis$LC.SLOPE7)
# model7 <- lmer(daynum.mean ~ LC.SLOPE7 + year + (1 |
#                                                    site), phenology.gis)
# anova(model2, model7)
# 
# #######
...
# levels(phenology.gis$LC.SLOPE8)
# model8 <- lmer(daynum.mean ~ LC.SLOPE8 + year + (1 |
#                                                    site), phenology.gis)
# anova(model2, model8)
# 
# #######
...
# levels(phenology.gis$LC.SLOPE9)
# model9 <- lmer(daynum.mean ~ LC.SLOPE9 + year + (1 |
#                                                    site), phenology.gis)
# anova(model2, model9)
# 
# #######
...
# levels(phenology.gis$LC.SLOPE10)
# model10 <- lmer(daynum.mean ~ LC.SLOPE10 + year + (1 |
#                                                      site), phenology.gis)
...
# levels(phenology.gis$LC.SLOPE11)
# model11 <- lmer(daynum.mean ~ LC.SLOPE11 + year + (1 |
#                                                      site), phenology.gis)
...
# levels(phenology.gis$LC.SLOPE12)
# model12 <- lmer(daynum.mean ~ LC.SLOPE12 + year + (1 |
#                                                      site), phenology.gis)
...
# levels(phenology.gis$LC.SLOPE13)
# model13 <- lmer(daynum.mean ~ LC.SLOPE13 + year + (1 |
#                                                      site), phenology.gis)
...
# levels(phenology.gis$LC.SLOPE14)
# model14 <- lmer(daynum.mean ~ LC.SLOPE14 + year + (1 |
#                                                      site), phenology.gis)
...
#with(phenology.gis, boxplot(daynum.mean ~ LC.SLOPE2, xlab = "Site type",
