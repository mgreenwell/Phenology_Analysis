...
library(lme4)
model<-lmer(daynum.mean~LC.DEM+year+(1|site),phenology.gis)

# (1|site) gives site as random effect- not a factor of interest
summary(model)
names(phenology.gis)
par(mfrow=c(1,1))
with(phenology.gis, boxplot(daynum.mean~LC.DEM))
...
#Testing whether LC.DEM has an effect on daynum.mean by comparing with a model that just looks at daynum.mean vs year with site as random effect
...
levels(phenology.gis$LC.DEM2)
model2<-lmer(daynum.mean~LC.DEM2+year+(1|site),phenology.gis)
anova(model,model2)
...
levels(phenology.gis$LC.DEM3)
model3<-lmer(daynum.mean~LC.DEM3+year+(1|site),phenology.gis)
anova(model,model3)
...
levels(phenology.gis$LC.DEM4)
model4<-lmer(daynum.mean~LC.DEM4+year+(1|site),phenology.gis)
anova(model,model4)
...
levels(phenology.gis$LC.DEM5)<-c("B","C","A","A")           ##("highlc.high alt","high lc.low alt","low lc.high alt and low lc.low alt","low lc.high alt and low lc.low alt")
levels(phenology.gis$LC.DEM5)
model5<-lmer(daynum.mean~LC.DEM5+year+(1|site),phenology.gis)
anova(model,model5)
...
levels(phenology.gis$LC.DEM6)
model6<-lmer(daynum.mean~LC.DEM6+year+(1|site),phenology.gis)
anova(model5,model6)
...
levels(phenology.gis$LC.DEM7)
model7<-lmer(daynum.mean~LC.DEM7+year+(1|site),phenology.gis)
anova(model5,model7)
...
levels(phenology.gis$LC.DEM8)
model8<-lmer(daynum.mean~LC.DEM8+year+(1|site),phenology.gis)
anova(model5,model8)
...
levels(phenology.gis$LC.DEM9)
model9<-lmer(daynum.mean~LC.DEM9+year+(1|site),phenology.gis)
anova(model5,model9)
...
levels(phenology.gis$LC.DEM10)
model10<-lmer(daynum.mean~LC.DEM10+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM11)
model11<-lmer(daynum.mean~LC.DEM11+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM12)
model12<-lmer(daynum.mean~LC.DEM12+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM13)
model13<-lmer(daynum.mean~LC.DEM13+year+(1|site),phenology.gis)
...
levels(phenology.gis$LC.DEM14)
model14<-lmer(daynum.mean~LC.DEM14+year+(1|site),phenology.gis)
...
with(phenology.gis, boxplot(daynum.mean~LC.DEM5,xlab="Site type",ylab="daynum.mean",ylim = c(0, 200)))
...
p<-ggplot(phenology.gis,aes(x=LC.DEM5,y=daynum.mean))+geom_violin(fill="grey",col="black")+theme_minimal()
